...
- 使用 Node 24、干净 `npm ci`、仓库 test-mode E2E harness 提供的已恢复 `leanote_test` MongoDB 5.0 fixture、每次运行随机 token 和随机化 fixture admin 凭据；不满足前置条件即失败，不创建替代服务或凭据。该 harness 供应路径必须在 fork PR 可用，不能读取 E2E GitHub Secrets。
...
- [ ] 在 `.github/workflows/regression-baseline.yml` 的同一 test-mode harness 中依次运行 `npm run test:e2e:build` 与 `npm run test:e2e`；为 business E2E 增加独立脱敏摘要、失败 artifact 和无条件 cleanup。任一身份、业务或清理失败必须阻断 PR/push。
