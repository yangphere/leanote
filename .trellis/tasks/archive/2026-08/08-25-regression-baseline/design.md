...
（harness 常量与 `[test]` section 各自引用同一值；`http.addr=127.0.0.1`、
...
watch=false
http.addr=127.0.0.1     # 测试可执行文件只监听回环，避免 Windows 防火墙放行弹窗
...
配置守卫还必须复刻 `db.Init` 的两个边界：URL 路径为空时使用已验证的
`[test] db.dbname`；续行值不得传入 `mgo.Dial`，因为续行拼接会保留字面换行并导致
启动时 panic，因此 `db.url` / `db.urlEnv` 含换行时要在启动前失败。
...
- 旧 `app/cmd` 路由生成在本机 Go 1.26.6 与 1.27.0 均会于固定版
  `golang.org/x/tools` 的 `go/types.(*StdSizes).Sizeof` 触发 nil panic。使用官方
