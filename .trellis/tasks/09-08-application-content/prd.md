...
- 当前上传、远程抓取、批量附件归档和 PDF 执行主要位于 controller；路径由 `revel.BasePath` 与数据库字符串拼接，尚无统一安全边界。
- 当前附件批量下载产物是 `tar.gz`，不是 ZIP；旧规格中的 ZIP 解压约束不适用。
- notes 已声明 `AssetMutation.Apply/Verify` 与 `ContentAssetPort`，但 content provider 尚未成为单一实现。
- `app/lea/html2image` 的有效实现被注释或直接返回固定值，且无业务调用/公开路由；它是 reachability/dead-code 项，不是待恢复能力。
...
- 原始文件名/title 去除路径、NUL/控制字符并设有界 UTF-8 长度；存储名由服务端生成。
...
- [ ] AC-C5：upload/remote decode 覆盖 multipart presence/one-file/stream size、invalid config fail-closed、`DecodeConfig`、width/height/pixel/allocation/frame budget、真实 GIF/JPEG/PNG/BMP、伪扩展/截断/后续帧损坏、metadata、cleanup、foreign owner。
...
- [ ] AC-C8：Web/API 单附件和全量 `tar.gz` 覆盖安全/重复 entry、真实内容、close order、concurrency、无共享 temp、失败无 panic/截断成功。
- [ ] AC-C9：公网限定 fetch policy 覆盖 scheme/userinfo、DNS/connection/redirect 逐跳重验、private/loopback/link-local/metadata/特殊地址、timeout/redirect/size cap、non-2xx、invalid image 和 cleanup；实现中不存在内网 allowlist 或默认放行开关。
...
- [ ] AC-C11：Web/API 共用 PDF renderer；不可信 note HTML 经 self-contained serializer，用户主动内容被移除，资源先授权/受限后内联；descriptor consumer、direct argv、stdin/0600、timeout/cancel、bounded stderr、PDF validation、cleanup、secret absence 与恶意 URL/active-content 的结构化拒绝 focused tests 通过。真实 local-file deny/零 outbound 运行证据按 AC-C14 交接，不由 fake executable 冒充。
...
规格审核已形成可执行结构，D-C1 已确认并写回。向用户提交本版最终规划摘要并获得后续明确开发批准前，任务保持 `planning`，不得运行 `task.py start` 或进入业务编码。
