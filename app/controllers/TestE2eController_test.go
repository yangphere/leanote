...
func TestEvaluateE2eIdentityFailClosed(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "future marker at one minute boundary is allowed", status: 200, runMode: "test", host: "127.0.0.1", dbName: "leanote_test", token: token, markers: []e2eRunMarker{e2eIdentityMarker(e2eIdentityNow.Add(time.Minute), valid, "browser-e2e")}},
		{name: "future marker beyond one minute boundary is 503", status: 503, runMode: "test", host: "127.0.0.1", dbName: "leanote_test", token: token, markers: []e2eRunMarker{e2eIdentityMarker(e2eIdentityNow.Add(time.Minute+time.Nanosecond), valid, "browser-e2e")}},
