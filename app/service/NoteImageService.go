...
func (this *NoteImageService) UpdateNoteImages(userId, noteId, imgSrc, content string) bool {
	...
	// 删除旧的
	db.DeleteAll(db.NoteImages, bson.M{"NoteId": db.MustObjectIDFromHex(noteId)})

	// 添加新的
	var fileId string
	noteImage := info.NoteImage{NoteId: db.MustObjectIDFromHex(noteId)}
			...
			if each != nil && len(each) == 3 {
				...
				if _, ok := hasAdded[fileId]; !ok {
					...
					if fileService.IsMyFile(userId, fileId) {
						noteImage.ImageId = db.MustObjectIDFromHex(fileId)
