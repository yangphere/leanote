...
Frontend assets are generated by the Node 24 pipeline (`npm ci && npm run build && npm test`).
`scripts/build/manifest.mjs` and `app/views/note/note-dev.html` are the sources of truth; generated
tracked assets must not be edited by hand.
...
- User-facing strings live in `messages/<locale>/*.conf` (7 locales). Server side reads them through
  `app/lea/i18n` (`msg`, `leaMsg`, `rawMsg` template funcs); client side bundles are generated by
...
(`public/js/app/*`, `public/js/plugins/*`), the markdown editor (`public/md`, requirejs), the self-hosted
TinyMCE 8 runtime with Leanote plugins (`leaui_image`, `leaui_mindmap`, `leanote_nav`,
...
- **`app/views/note/note-dev.html` is the source of truth** for the editor page; `note.html` is
  generated by `npm run build` (strips `<!-- dev -->` blocks, swaps in `*.min.js`). Edit only the
  dev file, then rebuild.
- **TinyMCE runtime assets are manifest-owned**: edit the source/configuration and run
  `npm run build`; do not restore the retired v4 `classes`, `paste`, `full`, theme, or skin copies.
  `tests/js/paste-plugin.test.js` verifies the v8 core and the single Leanote upload boundary.
